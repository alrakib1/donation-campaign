...
const Details = () => {
  ...
  const { picture, title, description, price,text_color
  } = detail;
  console.log(detail)
        ...
        <div className="md:relative">
          <div className="relative">
            ...
            <div className="md:absolute md:bottom-0 left-0 right-0 md:bg-[#0B0B0B80] md:h-32 flex justify-center items-center">
              <button className="btn mt-4 md:mt-0 md:absolute left-10 text-white border-0" style={{backgroundColor: text_color}}>Donate ${price}</button>
            </div>
          </div>
          <div className="md:w-[600px] lg:w-[1000px] mx-auto mt-12">
            <h3 className="mb-6 text-3xl md:text-4xl font-bold">{title}</h3>
            <p className="mb-24 text-[#0B0B0BB3] font-normal text-base">{description}</p>
          </div>
        </div>
      </div>
    </div>
